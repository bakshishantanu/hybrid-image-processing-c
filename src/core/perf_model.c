...
static double measure_bandwidth_gbps(void)
{
    ...
    {
        memcpy(dst, src, buf_size);
        sink += dst[buf_size - 1]; /* prevent optimizing away */
    }
    QueryPerformanceCounter(&t_end);
    seconds = (double)(t_end.QuadPart - t_start.QuadPart) / (double)freq.QuadPart;
#else
    ...
    if (seconds > 0.001)
    {
        /* Factor of 2: memcpy reads src AND writes dst */
        gbps = 2.0 * (double)buf_size * iters / seconds / (1024.0 * 1024.0 * 1024.0);
    }
